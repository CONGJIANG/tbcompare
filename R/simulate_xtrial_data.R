...
#'
#' @export
get_xtrial_dgp_parameters <- function(dgp_choice = "delta1") {
  delta_c <- switch(
...
#'
#' User-facing wrapper for generating cross-trial simulation data.
...
#' @param dgp_choice Character. One of \code{"delta0"}, \code{"delta1"},
#'   \code{"delta2"}, or \code{"delta3"}.
#' @param ... Additional arguments passed to \code{dgp_xtrial_data()}.
#'
#' @return A data frame with simulated cross-trial comparison data.
#'
#' @examples
...
#'
#' @export
simulate_xtrial_data <- function(
  n = 1000,
  dgp_choice = "delta1",
  ...
) {
  pars <- get_xtrial_dgp_parameters(dgp_choice = dgp_choice)
